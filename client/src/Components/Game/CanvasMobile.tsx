import { MutableRefObject, useContext, useLayoutEffect, useEffect, useRef, useState, useCallback } from 'react';
...
const Canvas = (props: Props) => {
  ...
  useLayoutEffect(() => {
    ...
    cvs.current.addEventListener('touchmove', movePaddleMobile);

    function movePaddle(e: MouseEvent) {
      var rect = ctx.canvas.getBoundingClientRect();
      const right = props.playerRight?.username === me.username;
      if ((e.clientY as unknown as string) !== 'undefined') {
        sockContext.socketPong.emit('mouseMoved', Math.round(((right ? rect.right - e.clientX : e.clientX - rect.left) - paddleHeight / 2) / ratio));
      }
    }

    // A TESTER
    function movePaddleMobile(e: TouchEvent) {
      var rect = ctx.canvas.getBoundingClientRect();
      const right = props.playerRight?.username === me.username;
      if ((e.touches[0].clientX as unknown as string) !== 'undefined') {
        sockContext.socketPong.emit('mouseMoved', Math.round(((right ? rect.right - e.touches[0].clientX : e.touches[0].clientX - rect.left) - paddleHeight / 2) / ratio));
      }
    }
    ...
    function update(match: Match) {
      ...
      playerRight.x = match.playerRight.x * ratio + paddleWidth / 2;
    ...
    function draw() {
      ...
      // score (mine and opp)
      // drawText(ctx, playerLeft.score.toString(), ctx.canvas.width / 4, ctx.canvas.height / 4, 'GREEN');
      // drawText(ctx, playerRight.score.toString(), ctx.canvas.width / 4, 3 * ctx.canvas.height, 'RED');
  ...
  const rotate = props.playerLeft?.username === me.username ? 'rotate(-90deg)' : 'rotate(90deg)';
  return (
    <React.Fragment>
      <canvas ref={cvs} height={screenWidth * PADDING * RATIO} width={screenWidth * PADDING} id="canvas-game" style={{ transform: rotate }} className='neon' />
    </React.Fragment>
  );
};
